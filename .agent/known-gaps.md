...
3. There is no RunnerStepDelta record that compares previous runner source state against current runner source state.
4. src/game.js can read PrehistoricRushHost.app, but it does not snapshot RunnerSourceState before applying direct presentation mutations.
5. applyCloseCamera still directly mutates the Three.js camera and does not leave behind a CameraFrameRequest.
6. renderHud still directly writes DOM and does not leave behind a HudFrameRequest.
7. applyReadableStride directly mutates the raptor rig and does not leave behind a DinoPoseFrame or pose consumer readback.
8. Contact checks still mutate scene/shards inline without a ContactResultSnapshot.
9. Scene dispatch still mutates menu/game/run-over/win inline without a SceneDispatchResult.
10. PrehistoricRushHost.getState() lacks a nested presentation snapshot.
11. Render readback is only the string renderer field, not structured frame consumption evidence.
12. Movement authority still lives inside the legacy visual runtime.
13. Jump, boost, turn, hazard, pickup, run-over, retry, and win behavior are not wrapped in stable action/result records.
14. Manifest files exist, but they are not yet the full runtime source of truth.
...
.agent/architecture-audit/2026-07-08T19-30-31-04-00-presentation-authority-consumer-dsk-map.md added
.agent/render-audit/2026-07-08T19-30-31-04-00-render-readback-consumer-fixture-map.md added
.agent/gameplay-audit/2026-07-08T19-30-31-04-00-runner-step-contact-scene-loop.md added
.agent/presentation-authority-audit/2026-07-08T19-30-31-04-00-consumer-fixture-acceptance-map.md added
.agent/deploy-audit/2026-07-08T19-30-31-04-00-static-fixture-validation-without-package-json.md added
.agent/trackers/2026-07-08T19-30-31-04-00/project-breakdown.md added
.agent/turn-ledger/2026-07-08T19-30-31-04-00.md added
...
The primary remaining gap is implementation of the pure `src/presentation/*` files, additive host projection, and DOM-free fixture. The first useful cut is the consumer bridge from live app state to `runner.moved`, because it activates the existing `dino-pose-domain-kit` instead of creating another parallel animation path.
