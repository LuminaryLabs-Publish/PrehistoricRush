...
app.innerHTML = `
  <section style="position:fixed;inset:0;background:#101b13;color:#f3e7ba;font:14px system-ui,sans-serif;overflow:hidden">
    <div id="prehistoric-render-host" style="position:absolute;inset:0"></div>
    <aside style="position:absolute;left:18px;top:18px;z-index:4;padding:12px 14px;border-radius:12px;background:#09110bcc;min-width:210px;pointer-events:none">
      <strong style="color:#ffd37a">Prehistoric Rush</strong>
      <div id="prehistoric-status" style="margin-top:7px;line-height:1.45">Loading Nexus World…</div>
    </aside>
  </section>`;
...
const [Nexus, Actor, Spatial, Interaction, Simulation, World, Presentation, Graphics, Animation, Render, THREE] = await Promise.all([
  ...
  import(RUNTIME_URLS.nexusSimulation),
...
const kits = [
  ...
  rootKit,
  createPrehistoricRushCourseDomainKit(Nexus, { seed: worldRecipe.seed, ...worldRecipe.route })
];
const engine = Nexus.createEngine({ kits });

if (!engine.n.world.getWorldDefinition(worldRecipe.id)) {
...
const visualMaterial = new THREE.MeshStandardMaterial({ color: 0x8b6d43, roughness: 0.9 });
const torso = new THREE.Mesh(new THREE.BoxGeometry(1.2, 1.1, 2.4), visualMaterial);
torso.position.y = 1.1;
playerVisual.add(torso);
const head = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.7, 1.0), visualMaterial);
...
function loop(now) {
  ...
  const descriptor = cameraFrame(state, dt);
  rendering.draw(state, descriptor);
  statusNode.innerHTML = `${worldRecipe.name}<br>${state.status} · ${Math.floor(state.distance)}m / ${worldRecipe.runtime.goalDistance}m<br>${state.speed.toFixed(1)} m/s · ${state.region}<br><small>Foundation terrain · vegetation OFF · ${world.landforms.length} landforms</small>`;
...
const getState = () => ({
  ...
  streamingReadiness: { foundationReady: true, rendererReady: true, vegetationRequired: false },
  treeFidelity: { disabled: true, packageCount: 0, counts: { near: 0, medium: 0, far: 0, horizon: 0 }, exactFrameAck: null },
  lushFoliage: { disabled: true, overflow: 0, nearCards: 0, mediumCards: 0 },
