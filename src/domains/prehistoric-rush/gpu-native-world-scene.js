...
struct Out { @builtin(position) position:vec4<f32>, @location(0) color:vec3<f32>, @location(1) worldPos:vec3<f32> };
...
@vertex fn vs_main(@builtin(vertex_index) index:u32)->Out{ let p=point(index); var out:Out; out.position=camera.viewProjection*vec4<f32>(p,1.0); let e=clamp((p.y+55.0)/150.0,0.0,1.0); let base=mix(vec3<f32>(0.12,0.27,0.12),vec3<f32>(0.39,0.43,0.27),e); let wet=clamp(0.17+sin(p.x*0.016+p.z*0.011)*0.1,0.0,0.34); out.color=mix(base,vec3<f32>(0.10,0.22,0.13),wet); out.worldPos=p; return out; }
@fragment fn fs_main(input:Out)->@location(0) vec4<f32>{ let d=distance(camera.cameraPosition.xyz,input.worldPos); let fog=1.0-exp(-0.000038*d*d); return vec4<f32>(mix(input.color,vec3<f32>(0.52,0.68,0.59),clamp(fog,0.0,0.94)),1.0); }`;
...
struct Out { @builtin(position) position:vec4<f32>, @location(0) color:vec3<f32>, @location(1) normal:vec3<f32>, @location(2) worldPos:vec3<f32> };
@vertex fn vs_main(@location(0) p:vec3<f32>,@location(1) n:vec3<f32>,@location(2) c:vec3<f32>,@builtin(instance_index) instanceIndex:u32)->Out { let i=instances[instanceIndex]; let model=mat4x4<f32>(i.m0,i.m1,i.m2,i.m3); let wp=model*vec4<f32>(p,1.0); var out:Out; out.position=camera.viewProjection*wp; out.worldPos=wp.xyz; out.normal=normalize((model*vec4<f32>(n,0.0)).xyz); out.color=c*i.tint.xyz; return out; }
@fragment fn fs_main(input:Out)->@location(0) vec4<f32>{ let light=0.48+max(0.0,dot(normalize(input.normal),normalize(vec3<f32>(-0.35,0.83,-0.28))))*0.66; var color=input.color*light; let d=distance(camera.cameraPosition.xyz,input.worldPos); let fog=1.0-exp(-0.000038*d*d); color=mix(color,vec3<f32>(0.52,0.68,0.59),clamp(fog,0.0,0.94)); return vec4<f32>(color,1.0); }`;

const GRASS_WGSL = `${COMMON_CAMERA}
@group(0) @binding(1) var<storage,read> grass:array<vec4<f32>>; struct Out{@builtin(position) position:vec4<f32>,@location(0) shade:f32,@location(1) worldPos:vec3<f32>};
@vertex fn vs_main(@builtin(vertex_index) vertexIndex:u32,@builtin(instance_index) instanceIndex:u32)->Out { let record=grass[instanceIndex]; let local=array<vec2<f32>,6>(vec2<f32>(-0.10,0.0),vec2<f32>(0.10,0.0),vec2<f32>(0.0,0.95),vec2<f32>(-0.10,0.0),vec2<f32>(0.0,0.95),vec2<f32>(-0.02,0.46)); let blade=local[vertexIndex]*record.w; let world=vec3<f32>(record.x+blade.x,record.y+blade.y,record.z); var out:Out; out.position=camera.viewProjection*vec4<f32>(world,1.0); out.shade=record.w; out.worldPos=world; return out; }
@fragment fn fs_main(input:Out)->@location(0) vec4<f32>{ var color=mix(vec3<f32>(0.17,0.40,0.14),vec3<f32>(0.36,0.66,0.25),input.shade); let d=distance(camera.cameraPosition.xyz,input.worldPos); let fog=1.0-exp(-0.000038*d*d); color=mix(color,vec3<f32>(0.52,0.68,0.59),clamp(fog,0.0,0.94)); return vec4<f32>(color,1.0); }`;

export async function createPrehistoricRushGPUWorldScene({ hostElement, world, recipe, gpuHost, computeHost, frameExecutor, rendering, contributions = [], onProgress = () => {} } = {}) {
  ...
  async function renderFrame(camera) {
    const cb = cameraBuffer(camera); await gpuHost.ensureResource({ id: ids.camera, type: "buffer", byteLength: cb.byteLength, usage: ["uniform", "copy-dst"] }, cb); uploadedBytes += cb.byteLength;
  ...
  await ensureDepth(); await ensureHeightfield({ x: 0, z: 0 }); await syncTrees({ x: 0, z: 0 }); await computeDense({ x: 0, z: 0 }); await renderFrame(rendering.camera);
  ...
  function scheduleFrame({ state, camera } = {}) {
    ...
    Promise.resolve().then(async () => {
      ...
      await renderFrame(camera); frameSequence += 1;
