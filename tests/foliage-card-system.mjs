...
import { getPrehistoricTreeFoliageTargets } from "../src/shared/prehistoric-tree-art-direction.js";
...
for (const archetype of PREHISTORIC_TREE_ARCHETYPES) {
  ...
  assert.equal(near.length, targets.near, `${archetype.id} uses the authored near target`);
  assert.equal(medium.length, targets.medium, `${archetype.id} uses the authored medium target`);
  assert.ok(near.length >= medium.length && medium.length >= 22, `${archetype.id} has readable near and medium card forms`);
  ...
  for (const cluster of descriptor.clusters) {
    assert.ok(familyIds.has(cluster.familyId), `${descriptor.id} is missing referenced family ${cluster.familyId}`);
  }
}
...
const broadModes = new Set(createTreeFoliageCardPlacements(broadCanopy, "near").map((entry) => entry.mode));
assert.ok(broadModes.has("canopy-core"));
assert.ok(broadModes.has("canopy-shell"));
assert.ok(broadModes.has("hanging-edge"));
...
console.log("chunky alpha-cutout foliage cards, complete family coverage, canopy zones, palms, ground cover, fidelity, and atmosphere contracts passed");
