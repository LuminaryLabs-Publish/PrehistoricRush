...
const runtimeSource = await readFile(new URL("../src/domains/prehistoric-rush/rendering-implementation.js", import.meta.url), "utf8");
...
assert.match(runtimeSource, /createWorkerPatchStreamingService/, "the active semantic renderer selects worker patch streaming");
assert.match(runtimeSource, /createThreeTreeFidelityLayer/, "worker patches feed the active tree fidelity presentation");
assert.match(runtimeSource, /workerStreaming\?\.update\(state\)/, "the render loop pumps bounded worker streaming");
assert.match(gameSource, /game-runtime-shared-gpu-v3\.js/, "the compatibility entry boots the semantic shared-GPU runtime");
